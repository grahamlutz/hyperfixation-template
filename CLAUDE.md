...
- **Do not let Sentry own the OpenTelemetry global.** Both inits carry `skipOpenTelemetrySetup: true`
  because every OTel global is first-one-wins and Sentry is up first in both processes — without it
  `registerLangfuse()`'s `provider.register()` is refused without a word, every `gen_ai` span is
  dropped at `tracesSampleRate: 0`, and Langfuse receives nothing from a real LLM call. What it
  costs is Sentry's own context manager, so `withScope` no longer forks: errors and the global
  handlers do not need it. `langfuse-otel-global.test.ts` fails on the missing option.
