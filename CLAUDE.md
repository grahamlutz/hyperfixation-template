...
| `tests/` | `contract` (the loop once, end to end), `flow-restart` (every flow twice), `draft-approval` (past the gate), `demo-draft-schema` (the validators alone), `records-archive` (the mixin), `workspace-render` (model output rendered as text), `inbox-render` (the same claim about the boxes that edit it), `inbox-decide` (one submission, one batch, one replay key), `board-render` (the board's columns, in the record type's order), `workspace-actor` (the session as an actor), `notify` (who a gate is told to, and the link), `admin-budget` (a member refused, an admin's value written and audited), `admin-budget-render` (the form's markup), `compose-envs` (the env contract), `compose-ports` (no service in the deploy file publishes a host port, and where the suites' overlay does), `email-transport` (which transport `SMTP_URL`'s scheme picks, the Cloudflare request, the notifier and the channel on the same sender, no URL in any throw, and nothing but `src/email.ts` importing `createTransport`), `instrumentation` (both processes' telemetry gates, and the `beforeSend` each one carries), `langfuse-otel-global` (the real Sentry beside the real `registerLangfuse()`: an error envelope, a `gen_ai` span reaching a localhost OTLP server with Langfuse's auth headers, and two concurrent requests' scopes staying apart over the context manager `initSentry()` installs), `scrub` (what is redacted, what is left readable, and the console wrapper), `passkey-enrol` (an already-registered authenticator told apart from every other refusal, and what each says), `llm-mode` (the mode the boot records: fixtures with no key, live with one), `admin-run-draft` (a member refused, one run started, a repeated key starting none), `admin-run-draft-render` (that form's markup), `core-version` (every installed `@hyperfixation/*` at one version, inside the range declared for it), `ci-image-gate` (the `image` job's template-only steps, gated on the marker a generated app does not have) |
| `tests/e2e/` | `pnpm test:e2e` only, against a served app and a real browser: `harness.ts` (the server, the browser, mailpit, sign-in), `exit-bar` (Phase 1's bar and the workspace), `demo-loop` (Phase 2's: the loop, two approvals with one edit, the send, pause and resume), `layers` (the standalone build, always: a page render and a server action in one process in both orders, twice, and a server log with no `DuplicateFlow` and no throw in it) |
...
| `src/otel-context.ts` | The one thing both inits do after `Sentry.init()`: put `SentryContextManager` on the OpenTelemetry context global, which `skipOpenTelemetrySetup` omits and `withScope` needs. See What not to do |
...
A generated app inherits this repo's CI verbatim, so those two steps — and the toolchain install
they need — are gated on `hashFiles('.hyperfixation-template')`, the marker `hf new` deletes.
Everything else in `image` runs in a generated app too, and `ci-image-gate.test.ts` fails if a
template-only step loses its gate.
...
- **Do not import `src/web.ts` from a route that only wants a connection.** It is the app, and so
  every `defineFlow` in it; `src/pool.ts` is the pool on its own. A route's graph is compiled once
  per module layer, so a route that carries the app for no reason carries a second copy of every
  registration in it — which is how `/auth/passkey` came to die on `DuplicateFlow` between its own
  server action and the re-render after it. `tests/e2e/layers.e2e.ts` is what holds this.
...
- **Do not publish a host port from `docker-compose.prod.yml`.** A box runs more than one app,
  and `ports: "3000:3000"` is why the second one could not start; the proxy reaches `web` over
  the Docker network through `expose`. `compose-ports.test.ts` fails on any `ports:` there — the
  prod suites layer `tests/e2e/docker-compose.e2e-ports.yml` for the port they need.
...
- **Do not let Sentry own the OpenTelemetry trace global, and do not let Langfuse own the context
  one.** Both inits carry `skipOpenTelemetrySetup: true` because every OTel global is first-one-wins
  and Sentry is up first in both processes — without it `registerLangfuse()`'s `provider.register()`
  is refused without a word, every `gen_ai` span is dropped at `tracesSampleRate: 0`, and Langfuse
  receives nothing from a real LLM call. But skipping the setup also omits `SentryContextManager`,
  and Sentry's async-context strategy is installed either way: with OTel's own context manager on the
  global, `withScope`/`withIsolationScope` put a forked scope nowhere and mutate the *process-global*
  default instead, so a tag set for one request rides out on every later request's event. So
  `initSentry()` calls `installSentryContextManager()` — `src/otel-context.ts`, both processes,
  before `registerLangfuse()` — and the two SDKs hold one global each.
  `langfuse-otel-global.test.ts` fails on either half.
